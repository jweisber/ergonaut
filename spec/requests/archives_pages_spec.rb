  ...
  context "when logged in as managing editor" do
    ...
    describe "show archived submission page" do
      ...
      it "can record the author's gender", js: true do
        ...
        sleep 1
        expect(page).not_to have_button("female_submit_btn")
        expect(accepted_submission.author.reload.gender).to eq 'Female'

        click_link("gender_popover_link")
        click_button("male_submit_btn")
        sleep 1
        expect(page).not_to have_button("male_submit_btn")
        expect(accepted_submission.author.reload.gender).to eq 'Male'

        click_link("gender_popover_link")
        click_button("unknown_submit_btn")
        sleep 1
        expect(page).not_to have_button("unknown_submit_btn")
        expect(accepted_submission.author.reload.gender).to eq nil
      end
